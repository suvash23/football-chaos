        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                const matchesPromise = supabase
                    .from('matches')
                    .select('id, round, home_team, away_team, kickoff_time, status, home_score, away_score, group_name, stadium, match_number, tournament')
                ...
                const mapped = dbMatches.map((m) => {
                    ...
                    const jsonMatch = (worldCup2026Raw.matches as unknown as JsonMatch[]).find(jm =>
                    ...
                    return {
                        ...
                        tournament: m.tournament || 'wc2026',
...
export async function fetchTopScorers(): Promise<Scorer[]> {
    const matches = (worldCup2026Raw.matches as unknown as JsonMatch[]);
...
export const WORLD_CUP_WINNERS: Record<string, WorldCupRecord> = {
    ...
    "Argentina": { winners: 3, runnersUp: 4, yearsWon: [1978, 1986, 2022], yearsRunnerUp: [1930, 1990, 2014, 2026] },
    ...
    "Spain": { winners: 2, runnersUp: 0, yearsWon: [2010, 2026], yearsRunnerUp: [] },
};
